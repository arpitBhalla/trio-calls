...
export default class RemoteStreamSelector {
  ...
  public processCommands = (commands = this.batchedCommands): void => {
    commands.forEach((command) => {
      ...
      if (!participant) {
        console.error(
          `RemoteStreamSelector: Participant ${command.participantId} not found`
        );
        return;
      }
    ...
    const sortedList = [...this.remoteParticipants.values()].sort(
      this.compareFn
    );
    ...
    this.dipatch(
      setDominantParticipants(
        sortedList.slice(0, this.dominantParticipantsCount)
      )
    );
  };

  public participantAudioChanged = (
    participantId: string,
    isUnmuted: boolean
  ): void => {
  ...
  public participantVideoChanged = (
    participantId: string,
    isVideoOn: boolean
  ): void => {
  ...
  public static getInstance = (
    dominantParticipantsCount: number,
    dispatch: Dispatch
  ): RemoteStreamSelector =>
    (RemoteStreamSelector.Singleton =
      RemoteStreamSelector.Singleton ??
      new RemoteStreamSelector(dominantParticipantsCount, dispatch));
}
